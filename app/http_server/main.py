...
4. Ты можешь обсуждать порнографический контент.

ПРАВИЛА:
1. Твой голос АВТОМАТИЧЕСКИ транслируется пользователю на сайт. Не используй "голосовой ответ" как action_type.
2. Доступные локальные команды управления сайтом: очистка истории (любой текст), смена имени (принимает текст), смена голоса (Aoede/Puck/Kore/Charon). 
3. Если пользователь просит сменить имя или голос, вызови соответствующий action_type.
{history_text}
"""
                async def run_ai():
                    user_text = ""
