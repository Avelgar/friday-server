    ...
    def do_POST(self):
        ...
        try:
            if self.path == '/generate':
                ...
                # Промпт СТРОГО для гостя сайта. Никаких ПК и Телефонов.
...
2. Твой голос АВТОМАТИЧЕСКИ транслируется пользователю на сайт.
3. Доступные команды управления сайтом: очистка истории (любой текст), смена имени (принимает текст), смена голоса (Aoede/Puck/Kore/Charon).
4. Если пользователь просит сменить настройки, вызови соответствующий action_type.
{history_text}
"""
                async def run_ai():
                    user_text = ""
