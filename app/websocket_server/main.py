...
CAP_TRIGGERS = "check_network_devices (найти устройства в сети), get_running_processes (получить список запущенных процессов устройства для их закрытия), get_installed_programs (получить пути к программам устройства для их открытия), request_retry (попросить пользователя повторить или уточнить)"

CAP_EXEC = "открытие файла (принимает ТОЧНЫЙ путь к .exe файлу из полученного списка программ), завершение процесса (принимает ТОЧНОЕ имя процесса из полученного списка процессов)"
...
6. Если команду невозможно выполнить без повторного запроса(не включая get_installed_programs и get_running_processes), то вызови триггер action_type="request_retry".
"""
...
3. Если нужно открыть файл/программу, отправляй action_type="get_installed_programs". Если закрыть — action_type="get_running_processes".
"""
