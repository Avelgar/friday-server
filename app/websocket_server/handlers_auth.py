...
async def handle_web_client_auth(websocket, data):
    ...
    try:
        ...
        if device: cursor.execute("UPDATE devices SET device_name = %s, is_online = TRUE WHERE mac = %s", (device_name, mac))
        else: cursor.execute("INSERT INTO devices (mac, device_name, password, is_online, user_id) VALUES (%s, %s, '123', TRUE, %s)", (mac, device_name, user_id))
...
async def handle_device_registration(websocket, data):
    ...
    try:
        ...
        if device:
            ...
            cursor.execute("INSERT INTO devices (mac, device_name, password, is_online) VALUES (%s, %s, %s, TRUE)", (mac, device_name, hashed_pw))
