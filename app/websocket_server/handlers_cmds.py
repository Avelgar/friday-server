...
ACTION_DESCRIPTIONS = {
    ...
    "delegate_to_robot_brain": "строго подробное описание задачи или название объекта. Вызывай для поиска объектов через камеру робота.",
...
АЛГОРИТМ РАБОТЫ:
1. Шаг 1 (ОБЯЗАТЕЛЬНЫЙ): Вызови инструмент "send_device_commands" с action_type="request_screenshot", чтобы запросить актуальное изображение экрана целевого устройства.
2. Шаг 2: Тебе придет картинка. Внимательно её изучи. У тебя АКТИВИРОВАН нативный инструмент "Computer Use".
3. Шаг 3 (ВЫБОР ДЕЙСТВИЯ): 
   - ВАРИАНТ А (Просто описание): Если просят найти что-то или описать — напиши подробный текст с ответом в action_value и вызови инструмент "send_device_commands" с action_type="task_completed".
   - ВАРИАНТ Б (Взаимодействие): Если нужно кликнуть мышью или напечатать текст — используй свои НАТИВНЫЕ встроенные функции (например, click, type, left_click). Моя система сама переведет их в действия на экране.
   - ВАРИАНТ В: После клика или ввода текста, ты можешь запросить еще один скриншот (Шаг 1), чтобы проверить результат, ИЛИ завершить работу через "task_completed".

ПРАВИЛО: Ты НЕ имеешь права завершать свою работу, пока не вызовешь "send_device_commands" с action_type="task_completed" и не опишешь результат в action_value!

Твои доступные кастомные инструменты (используй их внутри send_device_commands):
{heavy_caps}"""
...
- ФИЗИЧЕСКИЙ МИР (РОБОТ): Если просят робота (PiBot) найти объект в реальном мире, вызови "delegate_to_robot_brain" с названием объекта.

4. ШАГ 4 - ТИШИНА:
Пока собираешь данные — НЕ пиши текст, возвращай только вызов функции.
        ...
        async def device_bridge_callback(commands_to_execute, is_heavy=False):
            ...
            for cmd in commands_to_execute:
                ...
                for act in actions:
                    ...
                    # --- НОВЫЙ БЛОК ДЛЯ РОБОТА ---
                    elif act_type == "delegate_to_robot_brain" and not is_heavy:
                        logger.warning(f"🤖 [МОЗГ] Делегирует задачу Робо-Мозгу (ER): {act_val}")
                        robot_result = await ai_instance.execute_robotics_agent(
                            prompt_text=act_val,
                            device_bridge_callback=lambda cmds: device_bridge_callback(cmds, is_heavy=True),
                            model_id="gemini-robotics-er-2-preview" 
                        )
                        tool_resp["robot_brain_report"] = robot_result
                    # -----------------------------
