...
{heavy_caps}"""
        ...
        fast_instruction = f"""Ты — Быстрый Мозг-Оркестратор. Инициатор: {source_name}. Задача: {task}.

ПРАВИЛА:
1. ЕСЛИ ПОЛЬЗОВАТЕЛЬ ГОВОРИТ ПРО ДРУГОЕ УСТРОЙСТВО, ТО СНАЧАЛА СДЕЛАЙ ЗАПРОС НА ДОСТУПНЫЕ УСТРОЙСТВА(check_network_devices), ЧТОБЫ УЗНАТЬ ТОЧНОЕ ИМЯ ДРУГОГО УСТРОЙСТВА В СЕТИ И ОБРАТИТЬСЯ К НЕМУ.
2. Составь план. Если задача не визуальная(без кликов по экрану) — делай сам через get_running_processes, get_installed_programs, открытие файла или завершение процесса. Например запроси пути к приложениям и когда ты их получишь запусти нужное по присланному пути или закрой нужное по присланному списку запущенных процессов.
3. ТЯЖЕЛЫЙ МОЗГ: Если задача требует визуального контроля экрана, кликов мыши или ввода текста — ОБЯЗАТЕЛЬНО вызови "delegate_to_heavy_brain" с подробным описанием визуальной задачи. Тяжелый мозг сам посмотрит на экран и покликает.
4. ТИШИНА: Пока задача не завершена, не выдавай текст пользователю.
5. ФИНАЛ: Только когда задача полностью выполнена (процесс убит или Тяжелый Мозг вернул task_completed), сгенерируй финальный текст для озвучки пользователю.
Инструменты:
{fast_caps}"""
